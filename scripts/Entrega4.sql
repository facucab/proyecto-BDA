...
BEGIN
    ...
    BEGIN
        SELECT 'Error' AS Resultado, 'Los par�metros no pueden ser nulos' AS Mensaje;
        RETURN;
    END

    IF LTRIM(RTRIM(@cuit)) = ''
    BEGIN
        SELECT 'Error' AS Resultado, 'El CUIT no puede estar vac�o' AS Mensaje;
        RETURN;
    END

    IF LTRIM(RTRIM(@domicilio_comercial)) = ''
    BEGIN
        SELECT 'Error' AS Resultado, 'El domicilio no puede estar vac�o' AS Mensaje;
        RETURN;
    END
    ...
    BEGIN
        SELECT 'Error' AS Resultado, 
               'Las condiciones v�lidas frente al IVA son "Responsable Inscripto", "Monotributista" y "Exento"' AS Mensaje;
        RETURN;
    END

    IF LTRIM(RTRIM(@nombre)) = ''
    BEGIN
        SELECT 'Error' AS Resultado, 'El nombre de la empresa no puede estar vac�o' AS Mensaje;
        RETURN;
    END
...
BEGIN
    ...
    IF @lluvia < 0 OR @lluvia > 999.99
    BEGIN
        SELECT 'Error' AS Resultado, 'La cantidad de lluvia debe estar entre 0 y 999.99' AS Mensaje, '400' AS Estado;
        RETURN;
    END
    
    -- Valido que no exista ya un registro para esa fecha
    IF EXISTS (SELECT 1 FROM facturacion.clima WHERE fecha = @fecha)
    BEGIN
        SELECT 'Error' AS Resultado, 'Ya existe un registro de clima para esa fecha' AS Mensaje, '409' AS Estado;
        RETURN;
    END
